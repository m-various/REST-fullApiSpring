...
public class ProductApi {
    ...
    public ResponseEntity<List<UserProduct>> myProducts(@RequestParam(value = "userId") UUID userId) {
        Account a = usersRepo.findById(userId).orElse(null);

        if (userId == null) {
            return new ResponseEntity(HttpStatus.BAD_REQUEST);
        }
        //List<UserProduct> userProductList = userProductRepo.findAllByAccount_Id(userId);
        List<UserProduct> userProductList = a.getProducts();
    ...
    public ResponseEntity<ResponseBase> pay(@RequestBody PayRequest request) {
        ...
        try {
            //помещаем в лист все айдишники товаров
            ...
            Map<Product, Integer> uniqueProductMap = new HashMap<>();

            //при помощи каунтера помещаем уникальные продукты и количество их повторений в карту
            while (!productIds.isEmpty()) {
                UUID id = productIds.get(0);
                Product product = productRepo.findById(productIds.get(0)).orElse(null);
                countSameProduct = (int) productIds.stream().filter(uuid -> uuid.equals(id)).count();
                if (product != null) uniqueProductMap.put(product, countSameProduct);
                while (countSameProduct != 0) {
                    productIds.remove(0);
                    countSameProduct--;
                }
            }

            user = usersRepo.findById(request.getUserId()).orElse(null);

            //проверка юзера на null
            if (user == null) {
                return new ResponseEntity<ResponseBase>(new ResponseBase("Account not found."), HttpStatus.NOT_FOUND);
            }

            //если карта пустая, значит не найдено в базе ни одного продукта
            ...
            for (Map.Entry<Product, Integer> pair : uniqueProductMap.entrySet()) {
                Product keyProduct = pair.getKey();
                int valueCount = pair.getValue();
                discount = discountRepo.findByProductId(keyProduct.getId());
                if (discount != null) {
                    discountMoney = (long) (valueCount * (keyProduct.getPrice() * (discount.getDiscount() / 100f)));
            ...
            Long price = 0L;
            for (Map.Entry<Product, Integer> pair : uniqueProductMap.entrySet()) {
                price += (long)(pair.getKey().getPrice() * pair.getValue());
            }
            ...
            for (Map.Entry<Product, Integer> pair : uniqueProductMap.entrySet()) {
                ...
                int countValue = pair.getValue();
               // UserProduct userProduct = userProductRepo.findAllByAccount_IdAndProduct_Id(request.getUserId(), product.getId());

                UserProduct userProduct = user.getProducts().stream().filter(p -> p.getProductId().equals(product.getId())).findFirst().orElse(null);

                if (userProduct != null) {

                    userProduct.setAmount(userProduct.getAmount() + countValue);

                } else {

                    userProduct = new UserProduct(user.getId(), product.getId(), countValue);
